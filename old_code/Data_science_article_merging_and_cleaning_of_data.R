...
#As this project only include investigation of 4 columns these columns 
#selected and the rest is removed, to make the data smaller
#and thereby speed up the processing time
...
test <- test %>%  
  select(time_one_min,rainfall_mm,
         ...
         nitrate_effluent_mg_L,
         ammonium_effluent_mg_L)%>%
  mutate(HRT=volume/flow_effluent_m3_h)


test <- test %>% 
 # select(time_one_min,flow_effluent_m3_h) %>% 
  mutate(time_one_min=ymd_hms(time_one_min)+hours(28.4))



test <- test %>% 
  # select(time_one_min,flow_effluent_m3_h) %>% 
  mutate(time_one_min=ymd_hms(time_one_min)+hours(32))


test2 <- test %>% 
  as_tibble() %>% 
  mutate(time_day=floor_date(time_one_min,"day")) %>% 
  select(-time_one_min) %>% 
  group_by(time_day) %>% 
  summarise(rain_mm=sum(rainfall_mm),flow=mean(flow_effluent_m3_h))
...
volume=400+2400+740+2750+20000+13300+40
