...
average_over_time_interval <- function(df,
  ...
  df1 <- df %>% 
    ...
    select({{old_time_column_name}},{{rain_column_name}})%>% 
    ...
    summarise(Acc_rain_mm= sum(.data[[rain_column_name]])) %>% 
...
splitting_data_in_year_and_average_over_time_interval <- function(df,
             time_interval,
             old_time_column_name,
             rain_fall_column){
  ...
  df2018<- df %>% 
    filter_index("2018") %>% 
    average_over_time_interval(time_interval,
                               {{old_time_column_name}},
                               {{rain_fall_column}})
  df2019 <- df %>% 
    filter_index("2019") %>% 
...
data_day <- test %>% 
  filter_index("2018") %>% 
 data_day <-   average_over_time_interval(data_day, "1 day", hourly, rain)
data_day <- test %>% 
