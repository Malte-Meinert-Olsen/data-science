...
count_missing_values <- missing_rain_values %>% 
  mutate(Hour_time=floor_date(DATETIME,"hour")) %>% 
  select(-DATETIME, Rainfall_mm) %>%
  group_by(Hour_time) %>% 
...
#Setting the period from 2018-05-12 to 2018-05-27 to zero, as no rain was here
data_one_min <- data_one_min %>%  as_tsibble()


data_one_min %>% filter_index("2018-05-12")


data_one_min <- data_one_min
data_one_min %>% replace

test <- data_one_min

test <- test %>% filter_index("2018-05-12") %>% 
